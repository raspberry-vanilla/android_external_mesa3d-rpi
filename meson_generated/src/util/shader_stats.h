...
{
   return fprintf(fp, "%s shader: %" PRIu32 " instrs, %f cycles, %f fma, %f cvt, %f sfu, %f alu, %f v, %f t, %f ls, %" PRIu32 " code size, %" PRIu16 " threads, %" PRIu32 " loops, %" PRIu32 ":%" PRIu32 " spills:fills, %" PRIu32 " spill cost, %" PRIu32 " registers used, %" PRIu32 " uniforms used\n", prefix, stats->instrs, stats->cycles, stats->fma, stats->cvt, stats->sfu, stats->alu, stats->v, stats->t, stats->ls, stats->code_size, stats->threads, stats->loops, stats->spills, stats->fills, stats->spill_cost, stats->registers_used, stats->uniforms_used);
}

static inline void
valhall_stats_util_debug(struct util_debug_callback *debug, const char *prefix, const struct valhall_stats *stats)
{
   util_debug_message(debug, SHADER_INFO, "%s shader: %" PRIu32 " instrs, %f cycles, %f fma, %f cvt, %f sfu, %f alu, %f v, %f t, %f ls, %" PRIu32 " code size, %" PRIu16 " threads, %" PRIu32 " loops, %" PRIu32 ":%" PRIu32 " spills:fills, %" PRIu32 " spill cost, %" PRIu32 " registers used, %" PRIu32 " uniforms used", prefix, stats->instrs, stats->cycles, stats->fma, stats->cvt, stats->sfu, stats->alu, stats->v, stats->t, stats->ls, stats->code_size, stats->threads, stats->loops, stats->spills, stats->fills, stats->spill_cost, stats->registers_used, stats->uniforms_used);
}
...
#define vk_add_valhall_stats(out, stats) do { \
   ...
   vk_add_exec_statistic_f64(out, "ALU", "Estimated normalized ALU (Arithmetic Logical Unit) cycles", (stats)->alu); \
   vk_add_exec_statistic_f64(out, "Varying", "Estimated normalized Varying cycles", (stats)->v); \
   vk_add_exec_statistic_f64(out, "Texture", "Estimated normalized Texture cycles", (stats)->t); \
...
valhall_stats_serialize(uint8_t *dest, const struct valhall_stats *stats)
{
    ...
    memcpy(dest + offsetof(struct valhall_stats, alu), &stats->alu, sizeof(float));
...
#define vk_add_genisa_stats(out, stats) do { \
   ...
   vk_add_exec_statistic_u64(out, "Source hash", "Hash generated from shader source or vkd3d-proton provided.", (stats)->source_hash); \
